...
class Runner():
    def __init__(self, param):
        ...
        '''文件保存路径'''
        self.savename = self.p.savename
        if not self.p.wm: self.savename = self.p.model + '_' + time.strftime('%d_%m_%Y') + '_' + time.strftime(
            '%H:%M:%S')
        self.root = 'new_results/' + 'un-pretained/' + self.p.dataset + '/' + self.modelname
        self.PATH = os.path.join(self.root,
                                 'rank{:.0f}/lr{:.4f}/batch{:.0f}/emb_reg{:.5f}/time_reg{:.5f}/'.format(self.p.rank,
                                                                                                        ...
                                                                                                        self.p.time_reg))
        if self.p.wm:
            Param_path = self.savename
            self.load_model(os.path.join(self.PATH, Param_path))
            self.root = 'new_results/' + 'pretained/' + self.p.dataset + '/' + self.modelname
            self.PATH = os.path.join(self.root,
                                     'rank{:.0f}/lr{:.4f}/batch{:.0f}/emb_reg{:.5f}/time_reg{:.5f}/'.format(self.p.rank,
                                                                                                            self.p.learning_rate,
                                                                                                            self.p.batch_size,
                                                                                                            self.p.emb_reg,
                                                                                                            self.p.time_reg))
            self.savename = self.p.model + '_' + time.strftime('%d_%m_%Y') + '_' + time.strftime('%H:%M:%S')
        try:
    ...
    def add_model(self):
        ...
        """
        if self.modelname.lower() == 'complex':
            model = ComplEx(self.sizes, self.p.rank)
        elif self.modelname.lower() == 'tcomplex':
    ...
    def fit(self):
        ...
        for epoch in range(self.p.max_epochs):
            ...
            if self.p.wm:
                epoch_loss = optimizer.WM_epoch(self.dataset, examples, self.beta, self.temp, self.alpha)
            else:
                epoch_loss = optimizer.epoch(examples)
            ...
            if epoch < 0 or (epoch + 1) % self.p.valid_freq == 0:
                ...
                f.write("\n mask_best_epoch:")
                f.write(str(self.mask_best_epoch))
                f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--wm', default=False, action="store_true",
        help="Use the model pretrained by adaptive weight negative sample loss regulaarizer"
    )
    parser.add_argument(
